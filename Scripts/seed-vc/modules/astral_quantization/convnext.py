...
class InterpolationLayer(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, target_len: torch.Tensor, *args, **kwargs
    ) -> torch.Tensor:
            ...
            self.downsample_blocks = nn.ModuleList(
                [
                    ...
                    for _, downsample_factor in zip(
                        downsample_layer_indices, downsample_factors
                    )
                ]
            )
            ...
            self.upsample_blocks = nn.ModuleList(
                [
                    ...
                    for _, upsample_factor in zip(
                        upsample_layer_indices, upsample_factors
                    )
                ]
            )
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        for layer_idx, block in enumerate(self.blocks):
            if layer_idx in self.downsample_layer_indices:
                x = self.downsample_blocks[
                    self.downsample_layer_indices.index(layer_idx)
                ](x)
            if layer_idx in self.upsample_layer_indices:
                x = self.upsample_blocks[self.upsample_layer_indices.index(layer_idx)](
                    x
                )
            if layer_idx in self.interpolation_layer_indices:
                x = self.interpolation_blocks[
                    self.interpolation_layer_indices.index(layer_idx)
