...
class VoiceConversionWrapper(torch.nn.Module):
    ...
    def forward_cfm(
        self, content_indices_wide, content_lens, mels, mel_lens, style_vectors
    ):
        ...
        prompt_len = (
            (torch.rand([B], device=device) * prompt_len_max)
            .floor()
            .to(dtype=torch.long)
        )
    ...
    def forward_ar(self, content_indices_narrow, content_indices_wide, content_lens):
        ...
        for bib in range(content_indices_narrow.size(0)):
            reduced, reduced_len = self.duration_reduction_func(
                content_indices_narrow[bib]
            )
        ...
        duration_reduced_narrow_lens = torch.LongTensor(
            duration_reduced_narrow_lens
        ).to(device)
        ...
        loss = self.ar(
            cond, duration_reduced_narrow_lens, content_indices_wide, content_lens
        )
        return loss
    ...
    def forward(
        ...
        with torch.no_grad():
            _, content_indices_wide, content_lens = self.content_extractor_wide(
        ...
        if forward_ar:
            ...
            loss_ar = self.forward_ar(
                content_indices_narrow.clone(),
                content_indices_wide.clone(),
                content_lens,
            )
        else:
        ...
        if forward_cfm:
            ...
            loss_cfm = self.forward_cfm(
                content_indices_wide, content_lens, mels, mel_lens, style_vectors
            )
        else:
    ...
    def load_checkpoints(
        ...
        cfm_length_regulator_state_dict = self.strip_prefix(
            cfm_checkpoint["net"]["length_regulator"], "module."
        )
        cfm_state_dict = self.strip_prefix(cfm_checkpoint["net"]["cfm"], "module.")
        missing_keys, unexpected_keys = self.cfm.load_state_dict(
            cfm_state_dict, strict=False
        )
        missing_keys, unexpected_keys = self.cfm_length_regulator.load_state_dict(
            cfm_length_regulator_state_dict, strict=False
        )

        # ar
        ar_checkpoint = torch.load(ar_checkpoint_path, map_location="cpu")
        ar_length_regulator_state_dict = self.strip_prefix(
            ar_checkpoint["net"]["length_regulator"], "module."
        )
        ar_state_dict = self.strip_prefix(ar_checkpoint["net"]["ar"], "module.")
        missing_keys, unexpected_keys = self.ar.load_state_dict(
            ar_state_dict, strict=False
        )
        missing_keys, unexpected_keys = self.ar_length_regulator.load_state_dict(
            ar_length_regulator_state_dict, strict=False
        )
        ...
        content_extractor_narrow_checkpoint = torch.load(
            content_extractor_narrow_checkpoint_path, map_location="cpu"
        )
        ...
        content_extractor_wide_checkpoint = torch.load(
            content_extractor_wide_checkpoint_path, map_location="cpu"
        )
        ...
        # style encoder
        style_encoder_checkpoint_path = load_custom_model_from_hf(
            DEFAULT_SE_REPO_ID, DEFAULT_SE_CHECKPOINT, config_filename=None
        )
        style_encoder_checkpoint = torch.load(
            style_encoder_checkpoint_path, map_location="cpu"
        )
    ...
    @torch.no_grad()
    def compute_style(
        self, waves_16k: torch.Tensor, wave_lens_16k: torch.Tensor = None
    ):
        if wave_lens_16k is None:
            wave_lens_16k = torch.tensor([waves_16k.size(-1)], dtype=torch.int32).to(
                waves_16k.device
            )
        feat_list = []
        for bib in range(waves_16k.size(0)):
            feat = torchaudio.compliance.kaldi.fbank(
                waves_16k[bib : bib + 1, : wave_lens_16k[bib]],
                num_mel_bins=80,
                dither=0,
    ...
    def convert_timbre(
        ...
        source_wave_16k = librosa.resample(
            source_wave, orig_sr=self.sr, target_sr=16000
        )
        target_wave_16k = librosa.resample(
            target_wave, orig_sr=self.sr, target_sr=16000
        )
    ...
    def convert_voice(
        ...
        with torch.autocast(device_type=device.type, dtype=dtype):
            ...
            _, source_narrow_indices, _ = self.content_extractor_narrow(
                ...
                [source_wave_16k.size],
                ssl_model=self.content_extractor_wide.ssl_model,
            )
            _, target_narrow_indices, _ = self.content_extractor_narrow(
                target_wave_16k_tensor,
                [target_wave_16k.size],
                ssl_model=self.content_extractor_wide.ssl_model,
            )

            src_narrow_reduced, src_narrow_len = self.duration_reduction_func(
                source_narrow_indices[0], 1
            )
            tgt_narrow_reduced, tgt_narrow_len = self.duration_reduction_func(
                target_narrow_indices[0], 1
            )

            ar_cond = self.ar_length_regulator(
                torch.cat([tgt_narrow_reduced, src_narrow_reduced], dim=0)[None]
            )[0]
            ...
            # Length regulation
            cond, _ = self.cfm_length_regulator(
                ar_out, ylens=torch.LongTensor([ar_out_mel_len]).to(device)
            )
            (
                prompt_condition,
                _,
    ...
    def _process_content_features(self, audio_16k_tensor, is_narrow=False):
        ...
        content_extractor_fn = (
            self.content_extractor_narrow if is_narrow else self.content_extractor_wide
        )
        if audio_16k_tensor.size(-1) <= 16000 * 30:
            # Compute content features
            _, content_indices, _ = content_extractor_fn(
                audio_16k_tensor,
                [audio_16k_tensor.size(-1)],
                ssl_model=self.content_extractor_wide.ssl_model,
            )
        else:
            ...
            while traversed_time < audio_16k_tensor.size(-1):
                ...
                _, chunk_content_indices, _ = content_extractor_fn(
                    chunk,
                    [chunk.size(-1)],
                    ssl_model=self.content_extractor_wide.ssl_model,
                )
                if traversed_time == 0:
                    ...
                    features_list.append(
                        chunk_content_indices[:, 50 * overlapping_time :]
                    )
    ...
    def convert_voice_with_streaming(
        ...
        with torch.autocast(device_type=device.type, dtype=dtype):
            # Compute content features
            source_content_indices = self._process_content_features(
                source_wave_16k_tensor, is_narrow=False
            )
            target_content_indices = self._process_content_features(
        ...
        if convert_style:
            with torch.autocast(device_type=device.type, dtype=dtype):
                source_narrow_indices = self._process_content_features(
                    source_wave_16k_tensor, is_narrow=True
                )
                target_narrow_indices = self._process_content_features(
                    target_wave_16k_tensor, is_narrow=True
                )
            src_narrow_reduced, src_narrow_len = self.duration_reduction_func(
                source_narrow_indices[0], 1
            )
            ...
            for i in range(0, len(src_narrow_reduced), max_chunk_size):
                ...
                with torch.autocast(device_type=device.type, dtype=dtype):
                    ...
                    chunk_cond, _ = self.cfm_length_regulator(
                        chunk_ar_out,
                        ylens=torch.LongTensor([chunkar_out_mel_len]).to(device),
                    )
                    ...
                    if self.dit_compiled:
                        cat_condition = torch.nn.functional.pad(
                            ...
                            (
                                ...
                                0,
                                0,
                                self.compile_len - cat_condition.size(1),
                            ),
                            value=0,
                        )
                    # Voice Conversion
