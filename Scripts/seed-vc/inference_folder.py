...
def main(args):
    ...
    # Load target audio once
    ref_audio_orig = librosa.load(target_name, sr=sr)[0]
    ...
    sr_model = 22050 if not f0_condition else 44100
    hop_length = 256 if not f0_condition else 512
    max_context_window = sr_model // hop_length * 120
    overlap_frame_len = 16
    overlap_wave_len = overlap_frame_len * hop_length

    # Process target audio
    ref_audio = (
        torch.tensor(ref_audio_orig[: sr_model * 25]).unsqueeze(0).float().to(device)
    )
    ori_waves_16k = torchaudio.functional.resample(ref_audio, sr_model, 16000)
    ...
    for wav_file in tqdm(wav_files, desc="Processing recorded audios"):
        source_audio_orig = librosa.load(wav_file, sr=sr_model)[0]
        ...
        while processed_frames < cond.size(1):
            ...
            vc_wave = vocoder_fn(vc_target.float().to(vocoder_device)).squeeze()
        ...
        vc_wave_cat = np.concatenate(generated_wave_chunks)
        out_file_path = os.path.join(output_dir, os.path.basename(wav_file))
        torchaudio.save(
            out_file_path, torch.tensor(vc_wave_cat)[None, :].float(), sr_model
        )

        # Cleanup memory iteratively
        if torch.cuda.is_available():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source-dir", type=str, default="../RECORDED_AUDIOS")
    parser.add_argument("--target", type=str, default="../voices/male.wav")
    parser.add_argument("--output", type=str, default="./RECORDED_AUDIOS_CLONE")
