...
export default function Card(props) {
  ...
  const handleLangChange =
    side === "left" ? props.handleSrcLang : props.handleDstLang;
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const res = await api.post(API_ENDPOINTS.WORDS_ANALYZE, {
    ...
    return (
      <Popover>
          ...
          <PopoverTrigger asChild>
            <TooltipTrigger asChild>
              ...
              >
                <FontAwesomeIcon
                  icon={faInfoCircle}
                  className={`h-6 w-6 ${speakerColor === "#ffffff" ? "text-white" : "text-[#0a8cde]"}`}
                />
              </button>
            </TooltipTrigger>
          </PopoverTrigger>
        ...
        <PopoverContent
          side="bottom"
          align="end"
          className="w-80 p-4 bg-white/95 backdrop-blur-md border shadow-2xl rounded-xl z-50 text-black"
        >
            ...
            {wordInfo.length > 0 ? (
              wordInfo.map((w) => (
                <div key={w.id} className="mb-4 last:mb-0">
                  <strong className="text-lg text-blue-500 capitalize">
                    {w.text}
                  </strong>
                  {w.information?.other_ways_to_say?.length > 0 && (
                    <p className="text-sm mt-1">
                      <strong>Sinónimos:</strong>{" "}
                      {w.information.other_ways_to_say.join(", ")}
                    </p>
                  )}
                  {w.information?.additional_explanation && (
  ...
  return (
    <div
      ...
      style={
        side === "left"
          ? { backgroundImage: `url('/images/${VARIANT_LANG}-white.png')` }
          : { backgroundImage: `url('/images/${VARIANT_LANG}-2-blue.png')` }
      }
    >
      <div
        className={`flex h-[70px] w-[calc(100%-80px)] z-[1] animate-[fade-in_1.2s_cubic-bezier(0.390,0.575,0.565,1.000)_1.5s_both] ${
          side === "left"
            ? "max-[850px]:h-[25px]"
            : "w-full max-[850px]:h-[12px]"
        }`}
      >
        {side === "left" ? (
          <Image
          ...
          <div className="flex flex-row w-[calc(100%-80px)] h-[calc(60%-80px)] mt-[15px] relative">
            <div className="relative flex-1 h-full min-w-0">
              ...
              <Textarea
                ...
                placeholder={
                  lang?.code === "rap_Latn"
                    ? "Ka pāpaꞌi ꞌa ruŋa nei te vānaŋa mo huri"
                    : "Escriba aquí el texto a traducir"
                }
            ...
            {(showSpeaker || showClearLeft || isWordInformationActive) && (
              <div
                className="ml-2 flex flex-col items-center justify-start gap-2 pt-[10px] w-9 shrink-0"
                ref={containerRef}
              >
                {renderInfoPopover()}

                {showSpeaker &&
                  (separateGendersAllowed &&
                  showGenderOptions &&
                  !isSpeaking &&
                  !isLoadingAudio ? (
                    ...
                    <Tooltip delayDuration={1000}>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          aria-label={
                            isSpeaking ? "Detener lectura" : "Seleccionar voz"
                          }
